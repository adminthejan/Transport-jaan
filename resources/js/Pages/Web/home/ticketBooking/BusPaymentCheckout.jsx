...
import { Bus, CheckCircle, Clock, AlertCircle, CreditCard, RefreshCcw } from "lucide-react";
...
// Bus booking payments use payment_status values pending|paid|failed|cancelled|expired
// (see App\Models\BusBookingPayment via HasGatewayPaymentLifecycle).
...
const BusPaymentCheckout = ({ booking, checkout, pollingUrl, retryUrl, bookingListUrl }) => {
        ...
        <div>
            ...
            <section className="mx-auto w-full max-w-2xl px-4 md:px-6 py-12 md:py-16">
                <div className="text-center mb-10">
                    <Bus className="w-10 h-10 text-[#0955AC] mx-auto mb-3" />
                    <h1 className="text-3xl md:text-4xl font-extrabold text-[#0F172A]">Bus Booking Payment</h1>
                    <p className="text-[#64748B] mt-2">Ref: {live.bookingReference}</p>
                    ...
                    {live.seatNumbers?.length > 0 && (
                        <p className="text-[#64748B] text-[13px] mt-1">
                            Seats: {[...live.seatNumbers].join(", ")}
                            {live.returnLeg?.seatNumbers?.length ? ` · Return: ${live.returnLeg.seatNumbers.join(", ")}` : ""}
                        </p>
                    )}
...
export default BusPaymentCheckout;
