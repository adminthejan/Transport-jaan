...
const SeaVehicleSearchInner = ({ vehicleId: vehicleIdProp, vehicle: vehicleProp }) => {
  ...
  const getQuote = async () => {
    ...
    try {
      const { data } = await axios.get(route("client.seaBookings.quote"), {
        params: {
  ...
  const continueToCheckout = async () => {
    ...
    try {
      ...
      router.visit(route("client.seaBookings.checkout"), {
        method: "get",
        data: {
          vehicle_id: vehicleId,
    ...
    <div className="px-5 xl:px-0">
      ...
      <div className="poppins w-auto h-auto xl:w-[440px] xl:h-auto bg-[#F4F3F3] rounded-[19px] flex flex-col gap-10 py-10 xl:px-20 px-10">
        ...
        <div className="poppins text-[12px] w-full h-auto bg-[#0955AC0D] rounded-[5px] flex flex-col py-10 px-10">
          ...
          <h1 className="font-[600] mt-5 text-[#000000D9]">Skipper</h1>
          ...
          {needsDriver && (
            <p className="mt-3 text-[12px] font-[700] text-[#0955AC] bg-[#0955AC1A] border border-[#0955AC]/30 rounded-[6px] px-3 py-2">
              A skipper adds {(DRIVER_FEE_RATE * 100).toFixed(0)}% of the daily rate for chauffeur service.
            </p>
          )}
...
const SeaVehicleSearch = (props) => (
  <LocaleProvider>
    <SeaVehicleSearchInner {...props} />
  </LocaleProvider>
);

export default SeaVehicleSearch;
