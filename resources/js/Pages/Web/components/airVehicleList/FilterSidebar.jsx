...
import { Check, Fuel, Gauge, Plane, SlidersHorizontal, Tag, Users, X } from "lucide-react";

const BODY_TYPES = [
  { id: "private_jet", label: "Private Jet" },
  { id: "commercial_airliner", label: "Commercial Airliner" },
  { id: "helicopter", label: "Helicopter" },
  { id: "charter_aircraft", label: "Charter Aircraft" },
  { id: "light_aircraft", label: "Light Aircraft" },
  { id: "business_jet", label: "Business Jet" },
  { id: "turboprop_aircraft", label: "Turboprop Aircraft" },
  { id: "glider", label: "Glider" },
  { id: "seaplane", label: "Seaplane" },
  { id: "cargo_aircraft", label: "Cargo Aircraft" },
  { id: "hot_air_balloon", label: "Hot Air Balloon" },
...
const BRANDS = [
  ...
  { id: "cessna", label: "Cessna" },
  { id: "bell", label: "Bell" },
  { id: "gulfstream", label: "Gulfstream" },
  { id: "embraer", label: "Embraer" },
  { id: "dassault", label: "Dassault Falcon" },
  { id: "bombardier", label: "Bombardier" },
  { id: "atr", label: "ATR" },
  { id: "pilatus", label: "Pilatus" },
];
...
const FilterSidebar = ({ searchParams, onSearch }) => {
  ...
  const runSearch = (nextParams) => {
    ...
    router.get('/airVehicleList', nextParams, {
      preserveState: true,
      preserveScroll: true,
      replace: true,
    });
  ...
  return (
    <>
      ...
      >
        ...
        <FilterSection icon={<Plane className="w-4 h-4 text-[#0955AC]" />} title="VEHICLE TYPE" count={selectedBodyType.length}>
          {BODY_TYPES.map((type) => (
            <FilterOption
              key={type.id}
              label={type.label}
        ...
        <FilterSection icon={<Users className="w-4 h-4 text-[#0955AC]" />} title="CAPACITY" count={minSeats > SEATS_FLOOR ? 1 : 0}>
          <MinValueSlider min={SEATS_FLOOR} max={SEATS_CEILING} value={minSeats} suffix="Seats" onCommit={handleSeatsCommit} />
        </FilterSection>
