...
import { Fuel, Gauge, Heart, Plane as PlaneIcon, Settings2, Users } from "lucide-react";
...
const AirVehicleListContent = ({ vehicles: initialVehicles, authUser, likedVehicleIds, searchParams }) => {
    ...
    <div className="w-full py-6 xl:py-0">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <p className="bebas-neue text-[24px] md:text-[32px] font-[400]">
          we found <span className="text-[#0955AC]">{vehicles.length} Planes</span> for you
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        {sortedVehicles.map((v) => {
          ...
          const title = [v.manufacturer, v.model].filter(Boolean).join(" ") || v.model || "Aircraft";
      ...
      {vehicles.length === 0 && (
        <div className="flex flex-col items-center justify-center text-center py-20 bg-white rounded-2xl border border-gray-100">
          <PlaneIcon className="w-10 h-10 text-gray-300 mb-3" />
          <p className="text-[#0F0F0F] font-[600]">No aircraft match your filters</p>
...
export default AirVehicleListContent;
