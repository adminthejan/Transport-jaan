...
import { Fuel, Gauge, Heart, Ship as ShipIcon, Users } from "lucide-react";
...
const SeaVehicleListContent = ({ vehicles: initialVehicles, authUser, likedVehicleIds, searchParams }) => {
    ...
    <div className="w-full py-6 xl:py-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        {sortedVehicles.map((v) => {
          const specs = [];
          if (v.landSpec?.transmission_type) specs.push({ icon: ShipIcon, label: formatLabel(v.landSpec.transmission_type) });
      ...
      {vehicles.length === 0 && (
        <div className="flex flex-col items-center justify-center text-center py-20 bg-white rounded-2xl border border-gray-100">
          <ShipIcon className="w-10 h-10 text-gray-300 mb-3" />
