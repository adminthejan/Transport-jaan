...
const SearchForm = ({ formData, onFormChange, onSearch, redirectToFirstVehicle = false }) => {
  ...
  const handleSearch = async () => {
    ...
    // Otherwise navigate to seaVehicleList page with form data as query parameters
    ...
    if (formData.withSkipper) params.set('withSkipper', 'true');

    const queryString = params.toString();

    if (redirectToFirstVehicle) {
      try {
        const response = await fetch(queryString ? `/seaVehicleList/json?${queryString}` : '/seaVehicleList/json');
        ...
        if (firstVehicle?.id) {
          router.visit(`/seaVehicleDetails/${firstVehicle.id}${queryString ? `?${queryString}` : ''}`, {
            method: 'get',
            preserveScroll: true,
          });
          return;
        ...
        console.error('Failed to find a sea vehicle:', error);
      }
    }

    const url = queryString ? `/seaVehicleList?${queryString}` : '/seaVehicleList';
      ...
      <div className="figtree bg-white p-4 sm:p-6 rounded-[20px] border border-black/5 shadow-[0_16px_40px_rgba(9,85,172,0.12)] w-full min-h-[132px] text-[#286BB6] text-[13px] font-[400]">
        ...
        <div className="flex flex-col sm:flex-row items-end gap-4">
          ...
          <div className="w-full sm:w-auto shrink-0">
            <label className="block mb-1.5 text-[#0F0F0F] font-[600] text-[12px]">Skipper</label>
            <div className="inline-flex w-full sm:w-auto rounded-[10px] border border-[#0000001A] bg-[#F1F5F9] p-1">
              {[
                { value: false, label: "Without Skipper" },
                { value: true, label: "With Skipper" },
              ].map((opt) => (
                <button
                  type="button"
                  key={String(opt.value)}
                  onClick={() => onFormChange({ ...formData, withSkipper: opt.value })}
                  className={`flex-1 sm:flex-none whitespace-nowrap rounded-[8px] px-3 sm:px-4 h-[46px] text-[12px] font-[700] transition-colors cursor-pointer ${
                    Boolean(formData.withSkipper) === opt.value
