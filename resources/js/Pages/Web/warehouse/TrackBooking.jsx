...
import { Search, Warehouse, CheckCircle, Clock, AlertCircle, Mail, KeyRound, ShieldCheck, Calendar, Wallet } from "lucide-react";
...
const TrackBooking = ({ reference: initialReference, email: initialEmail, result, notFound, needsVerification }) => {
    ...
    const handleSubmit = (e) => {
        ...
        router.get("/track-warehouse-booking", params, { preserveScroll: true });
    };
        ...
        <div>
            ...
            <section className="mx-auto w-full max-w-3xl px-4 md:px-6 py-12 md:py-16">
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl md:text-4xl font-extrabold text-[#0F172A]">Track Your Warehouse Booking</h1>
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#EEF2F6] shadow-sm p-5 md:p-6 mb-10 space-y-3">
                    <div className="relative">
                        <Warehouse className="absolute left-4 top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-[#0955AC] pointer-events-none" />
                        <input
                            type="text"
                            value={reference}
                            onChange={(e) => setReference(e.target.value)}
                            placeholder="Booking reference, e.g. WHS-XXXXXX-YY"
                ...
                {result && (
                    <div className="bg-white rounded-2xl border border-[#EEF2F6] shadow-sm p-6 md:p-8">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-6 pb-6 border-b border-[#F1F5F9]">
                            <div>
                                <p className="text-[12px] text-[#94A3B8] font-[600]">Reference &middot; Warehouse Storage</p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {result.warehouseName && (
                                <div className="flex items-start gap-2">
                                    <Warehouse className="w-4 h-4 text-[#0955AC] mt-0.5" />
                                    <div>
                                        <p className="text-[12px] text-[#94A3B8] font-[600]">Warehouse</p>
                                        <p className="text-[14px] font-[700] text-[#0F172A]">{result.warehouseName}</p>
                                    </div>
                                </div>
                            )}
                            {result.startDate && (
                                <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                        <p className="text-[12px] text-[#94A3B8] font-[600]">Move-in</p>
                            ...
                            {result.endDate && (
                                <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                        <p className="text-[12px] text-[#94A3B8] font-[600]">Move-out</p>
                                        <p className="text-[14px] font-[700] text-[#0F172A]">
                            ...
                            {result.finalAmount != null && (
                                <div className="flex items-start gap-2">
                                    <Wallet className="w-4 h-4 text-[#0955AC] mt-0.5" />
                                    <div>
                                        <p className="text-[12px] text-[#94A3B8] font-[600]">Total</p>
                                        <p className="text-[14px] font-[700] text-[#0F172A]">
                                            {result.currency || ""} {Number(result.finalAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                        </p>
                                    </div>
                                </div>
                            )}
