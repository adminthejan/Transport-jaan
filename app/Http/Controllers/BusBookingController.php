...
use App\Models\BusBookingPayment;
...
class BusBookingController extends Controller
{
    ...
    public function store(Request $request, WalletService $wallets)
    {
        ...
        try {
            ...
            // outbound leg with a failed return leg. The payment step (wallet
            // debit or a pending PayHere payment row) happens inside the same
            // transaction, so a wallet InsufficientWalletBalanceException (or
            // any other failure) rolls back the seat reservations too instead
            // of leaving an orphaned pending booking with no way to pay.
            $result = DB::transaction(function () use ($request, $passengerData, $seatNumbers, $returnSeatNumbers, $seatGenders, $returnSeatGenders, $isRoundTrip, $paymentMethod, $wallets) {
                ...
                $combinedTotal = (float) $outbound->total_price + ($return ? (float) $return->total_price : 0.0);
                ...
                if ($paymentMethod === 'Wallet') {
                    ...
                    $walletTx = $wallets->debit(
                        ...
                        $combinedTotal,
                        'payment',
                        'Bus booking #' . $outbound->booking_reference,
                        'BusBooking',
                        $outbound->id
                    );

                    $payment = BusBookingPayment::create([
                        'bus_booking_id' => $outbound->id,
                        'method' => 'Wallet',
                        'option' => 'full',
                        'amount_paid' => $combinedTotal,
                        'status' => BusBookingPayment::STATUS_PAID,
                    ...
                    BusBookingPayment::create([
                        'bus_booking_id' => $outbound->id,
                        'method' => 'PayHere',
                        'option' => 'full',
                        'amount_paid' => $combinedTotal,
                        'status' => BusBookingPayment::STATUS_PENDING,
                        'provider' => 'payhere',
                        'gateway_order_id' => $this->generateBusPaymentOrderId(),
                        'initiated_at' => now(),
                    ]);
                }
            ...
            if ($paymentMethod === 'PayHere') {
                $redirectUrl = route('bus-bookings.payhere.checkout', ['booking' => $booking->id]);
                $message = 'Booking created. Continue with PayHere to complete your payment.';
            } else {
                $redirectUrl = route('bus.booking.success', $booking->booking_reference);
            ...
            Log::warning('Bus booking wallet debit failed: insufficient balance', ['user_id' => Auth::id()]);
            ...
            return back()->withErrors(['payment_method' => $message])->withInput();
        } catch (ValidationException $e) {
    ...
    /**
     * Render the PayHere checkout step for a bus booking (or round-trip pair)
     * created with payment_method = 'PayHere'. Mirrors
     ...
     */
    public function paymentCheckout(Request $request, BusBooking $booking, PayHereGatewayService $gateway)
    {
        ...
        $checkout = $payment->isPending()
            ? $this->buildBusCheckout($gateway, $booking, $payment)
            : ['isReady' => false, 'reason' => null, 'checkoutUrl' => null, 'fields' => []];

        $payload = [
            'booking' => $this->serializeBusBookingForCheckout($booking, $payment),
            'checkout' => $checkout,
            'pollingUrl' => route('bus-bookings.payhere.status', ['booking' => $booking->id]),
            'retryUrl' => route('bus-bookings.payhere.retry', ['booking' => $booking->id]),
            'bookingListUrl' => route('bus.booking.success', $booking->booking_reference),
        ];
        ...
        return Inertia::render('Web/home/ticketBooking/BusPaymentCheckout', $payload);
    }
     ...
     */
    public function paymentStatus(BusBooking $booking)
    {
        abort_unless(Auth::check() && (int) $booking->user_id === (int) Auth::id(), 403);
        ...
        return response()->json(['booking' => $this->serializeBusBookingForCheckout($booking, $payment)]);
    }
     ...
     */
    public function paymentRetry(Request $request, BusBooking $booking)
    {
        abort_unless(Auth::check() && (int) $booking->user_id === (int) Auth::id(), 403);
        ...
        if ($payment->isPaid()) {
            return response()->json([
                ...
                'redirect' => route('bus-bookings.payhere.checkout', ['booking' => $booking->id]),
            ]);
        }
    ...
    public function handlePayHereReturn(Request $request)
    {
        ...
        $payment = BusBookingPayment::where('gateway_order_id', $orderId)->first();

        if (!$payment) {
            return redirect()->route('busTicketBookingDetails.busTicketBookingDetails')->with('error', 'We could not find that booking payment.');
        }

        $booking = BusBooking::find($payment->bus_booking_id);
        if (!$booking) {
            return redirect()->route('busTicketBookingDetails.busTicketBookingDetails')->with('error', 'We could not find that booking.');
        }
        ...
        if ($payment->isPending()) {
            ...
            if ($gateway->verifyNotifySignature($payload)) {
                $this->applyResolvedBusPaymentStatus($gateway, $payment, $booking, $payload);
            } elseif ($this->isBusPayHereSandbox() && $request->query('status_code') === null) {
                ...
                DB::transaction(function () use ($payment, $booking) {
                    $locked = BusBookingPayment::where('id', $payment->id)->lockForUpdate()->first();
                    if ($locked && $locked->isPending()) {
                        $locked->markPaid(null, null, 'sandbox-return');
                        $this->confirmBusBookingAfterPayment($booking);
                    }
                });
            }
        }

        return redirect()->route('bus-bookings.payhere.checkout', ['booking' => $booking->id]);
    }
    ...
    public function handlePayHereCancel(Request $request)
    {
        ...
        return redirect()->route('busTicketBookingDetails.busTicketBookingDetails')->with('error', 'Payment cancelled.');
    }
    ...
    public function handlePayHereNotify(Request $request)
    {
        ...
        $payment = BusBookingPayment::where('gateway_order_id', $orderId)->first();
        if (!$payment) {
            Log::warning('PayHere bus notify: unknown order_id', ['order_id' => $orderId]);
            return response('order not found', 404);
        }

        $booking = BusBooking::find($payment->bus_booking_id);
        if (!$booking) {
            Log::warning('PayHere bus notify: booking missing for payment', ['payment_id' => $payment->id]);
            return response('booking not found', 404);
        }

        $gateway = app(PayHereGatewayService::class);
        if (!$gateway->verifyNotifySignature($payload)) {
            Log::warning('PayHere bus notify: signature mismatch', ['order_id' => $orderId]);
    ...
    private function applyResolvedBusPaymentStatus(PayHereGatewayService $gateway, BusBookingPayment $payment, BusBooking $booking, array $payload): void
    {
     ...
     */
    private function confirmBusBookingAfterPayment(BusBooking $booking): void
    {
        $ids = [$booking->id];
        if ($booking->round_trip_group_id) {
            $ids = BusBooking::where('round_trip_group_id', $booking->round_trip_group_id)->pluck('id')->all();
        }

        BusBooking::whereIn('id', $ids)->update(['status' => 'confirmed', 'payment_status' => 'paid']);
    }

    private function buildBusCheckout(PayHereGatewayService $gateway, BusBooking $booking, BusBookingPayment $payment): array
    {
        $user = Auth::user();
        ...
        return $gateway->buildCheckoutPayload([
            ...
            'currency' => 'LKR',
            'items' => 'Bus booking #' . $booking->booking_reference,
            'returnUrl' => route('bus-bookings.payhere.return', ['order_id' => $payment->gateway_order_id]),
            'cancelUrl' => route('bus-bookings.payhere.cancel', ['order_id' => $payment->gateway_order_id]),
            'notifyUrl' => route('bus-bookings.payhere.notify'),
            'firstName' => $firstName,
            'lastName' => $lastName,
    ...
    private function serializeBusBookingForCheckout(BusBooking $booking, BusBookingPayment $payment): array
    {
        $booking->loadMissing('busSchedule.departureStation', 'busSchedule.arrivalStation');
        ...
        return [
            ...
            'returnLeg' => $partner ? [
                ...
                'seatNumbers' => $partner->seat_numbers,
            ] : null,
        ];
    }

    private function generateBusPaymentOrderId(): string
    {
        // Deliberately NOT the "BUS-" prefix used by booking_reference
        // (BookingReferenceGenerator::forBus()) — this is the PayHere
        // *payment* order id, a separate identifier space.
        return 'BUP-' . now()->format('ymd') . '-' . strtoupper(Str::random(8));
    }

    private function isBusPayHereSandbox(): bool
    {
